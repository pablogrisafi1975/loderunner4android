...
public class LodeRunnerDrawingThread extends Thread {
	...
	private static final String STORE_LEVEL_STATUSES = "levelStatuses";
	...
	public void stageOver(boolean hasCompleted) {
		...
		if (hasCompleted) {
			...
			pauseMessage = (level % LodeRunnerStage.GAME_LEVELS == 0) ? null : MSG_CONGRATS;
		} else {
			lives--;
			if (lives < 0) {
				lives = MAX_LIFES;
				level = 0;
				pauseMessage = MSG_GAME_OVER;
			} else {
				pauseMessage = MSG_TRY_AGAIN;
			}
		}
	...
	private void recoverStatus() {
		...
		this.level = settings.getInt(STORE_LEVELS, 0);
		this.lives = settings.getInt(STORE_LIVES, MAX_LIFES);
		String[] levels = settings.getString(STORE_LEVEL_STATUSES, "").split("L");
		for (int i = 0; i < levelStatuses.length; i++) {
	...
	private void persistStatus() {
		...
		editor.putInt(STORE_LEVELS, level);
		editor.putInt(STORE_LIVES, lives);
		StringBuffer levels = new StringBuffer();
		for (byte b : levelStatuses) {
			levels.append(String.valueOf(b)).append('L');
		}
		editor.putString(STORE_LEVEL_STATUSES, levels.toString());
